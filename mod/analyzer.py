...
def calculate_hot_concepts(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for concept in HOT_CONCEPT_LIST:
        ...
        score = avg_pct * 0.5 + (zhangt * 4)
        stats.append({
            '热门概念': concept, '个股数': count, '红盘率%': round(red_ratio, 1),'涨停%': round(zhangt, 1),
            '平均涨跌%': round(avg_pct, 1), '增量(亿)': round(net_inc, 1),
            '强度得分': round(score, 1), '增量先锋': top_stock,
...
def calculate_auto_concepts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_c = df.copy()
    df_c['涨跌幅_num'] = pd.to_numeric(df_c['涨跌幅'], errors='coerce').fillna(0)
    df_c['tag_list'] = (df_c['所属概念'].fillna('') + ';' + df_c['所属行业'].fillna('')).str.replace('，',
                                                                                                     ';').str.split(';')
