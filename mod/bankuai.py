...
def report_continuous_limit_up_acceleration(today_date: datetime, prev_date: datetime, df: pd.DataFrame) -> pd.DataFrame:
    ...
    condition = (
        ...
        ((昨竞<-2) & (连) & (今>5) & (倍数<3) & (倍数>2) & (昨成>1)) |   #赫美集团
        ((昨竞>-1) & (昨竞<0) & (昨收<3) & (今>9.8) & (昨成>0.1) & (昨成<0.2) & (倍数>5) & (倍数<10)) | #招商南油
        ((昨竞>9.8) & (今>9.8) & (昨成>0.7) & (倍数>1) & (倍数<2)) | #招商南油
        ((昨竞>1) & (昨竞<3) & (今>6) & (昨成>0.5) & (倍数>3) & (倍数<5)) | #招商轮船
        ((昨竞>6) & (今>5) & (昨成>1.5) & (倍数>0.2) & (倍数<0.8) & (昨收>9.8)) | #招商轮船
        ((昨竞>0) & (昨竞<1) & (今>9.8) & (昨成<0.1) & (倍数>15) & (倍数<25)) | #中远海轮
        ((昨竞>9.8) & (今>6) & (昨成>0.9) & (倍数>1) & (倍数<2))  #中远海轮
    )
